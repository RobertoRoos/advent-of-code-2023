...
def get_next_in_sequence(initial_sequence: List[int]) -> int:
    ...
    last_sequence.insert(0, 0)  # Add a new zero to the first row

    # Now work our way back up to find the next number of the initial sequence:
    for i in range(len(sequences_list) - 2, -1, -1):
        # Next value is the difference of the start of the seq. + the first value of the prev. seq.:
        new_val = sequences_list[i][0] - sequences_list[i + 1][0]
        sequences_list[i].insert(0, new_val)

    return sequences_list[0][0]  # Last new value
